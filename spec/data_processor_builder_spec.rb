...
describe DataProcessorBuilder do
	...
	it 'should provide data processors when raw data under new keys become available' do
		...
		data_processors = subject.classify(RawDataKey['nina', 'system/CPU usage/CPU/0', 'system'])
		data_processors.should have(1).data_processors
		data_processors.shift.should be_a DataProcessor

		data_processors = subject.classify(RawDataKey['nina', 'system/CPU usage/CPU/0', 'stolen'])
		data_processors.should have(1).data_processors
		data_processors.shift.should be_a DataProcessor

		data_processors = subject.classify(RawDataKey['magi', 'system/CPU usage/total', 'user'])
		data_processors.should be_empty

		data_processors = subject.classify(RawDataKey['magi', 'system/CPU usage/total', 'system'])
		data_processors.should have(1).data_processors
		data_processors.shift.should be_a DataProcessor
	end
	...
	describe DataProcessor do
		...
		it 'should have ID based on source of it' do
			...
			subject.classify(RawDataKey['magi', 'system/CPU usage/total', 'user']).should be_empty
			subject.classify(RawDataKey['magi', 'system/CPU usage/total', 'system']).first.id.should == 'system CPU usage:total:magi'
		end
		...
		it 'should have a tag set based on available raw data' do
			...
			subject.classify(RawDataKey['nina', 'system/CPU usage/CPU/0', 'system']).first.tag_set.should == TagSet[
				Tag.new('hello'), 
				Tag.new('world'), 
				Tag.new('location:nina'), 
			...
			subject.classify(RawDataKey['nina', 'system/CPU usage/CPU/0', 'stolen']).first.tag_set.should == TagSet[
				Tag.new('hello'), 
				Tag.new('world'), 
				Tag.new('location:nina'), 
				Tag.new('system:CPU usage:CPU:0'), 
			...
			subject.classify(RawDataKey['magi', 'system/CPU usage/total', 'system']).first.tag_set.should == TagSet[
				Tag.new('hello'), 
				Tag.new('world'), 
				Tag.new('location:magi'), 
		...
		it 'should have a proper key set' do
			...
			subject.classify(RawDataKey['nina', 'system/CPU usage/CPU/0', 'system']).first.raw_data_key_set.should == RawDataKeySet[
				RawDataKey['nina', 'system/CPU usage/CPU/0', 'user'],
				RawDataKey['nina', 'system/CPU usage/CPU/0', 'system']
			]

			subject.classify(RawDataKey['nina', 'system/CPU usage/CPU/0', 'stolen']).first.raw_data_key_set.should == RawDataKeySet[
				RawDataKey['nina', 'system/CPU usage/CPU/0', 'user'],
				RawDataKey['nina', 'system/CPU usage/CPU/0', 'system'],
			...
			subject.classify(RawDataKey['magi', 'system/CPU usage/total', 'user']).should be_empty
			subject.classify(RawDataKey['magi', 'system/CPU usage/total', 'system']).first.raw_data_key_set.should == RawDataKeySet[
