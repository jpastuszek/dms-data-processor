...
class DataProcessor
	...
	def initialize(data_type_name, id, raw_data_key_set, tag_set, &block)
	...
	def to_s
		"DataProcessor[#{@data_type_name}][#{@id}]<#{@tag_set.to_a.sort.map{|t| t.to_s}.join(', ')}>{#{@raw_data_key_set.to_a.map{|k| k.to_s}.sort.join(', ')}}"
	end
end
...
class DataProcessorBuilder
	class DataProcessorGroup
		...
		def initialize(data_type_name, builder_name, name, builder_tag_set)
		...
		def make_data_processor(group)
			...
			dp = DataProcessor.new(@data_type_name, dp_id, group.raw_data_key_set, group.tag_set, &@processor)
	...
	def initialize(name, data_type_name, &block)
		@name = name.to_s
		@data_type_name = data_type_name.to_s
